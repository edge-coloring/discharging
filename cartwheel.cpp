...
bool CartWheel::operator==(const CartWheel &right) const {
...
// (i) hub が rules に従って近傍から charge を送受した結果 threshold を超えたかどうかの判定結果
// (ii) cartwheel の頂点でルールを送るのに関係しているかどうかを表す bool 配列。
//　を返す。
pair<bool, vector<bool>> CartWheel::isOvercharged(const vector<Rule> &rules, int threshold) const {
    int hub = 0;
    int hub_degree = numNeighbor();
    ...
    return make_pair(charge > threshold, is_rule_related);
}
...
void searchOverChargedCartWheel(
    ...
    const vector<Configuration> &reducible_confs, int max_degree, int threshold) {
    auto base_cartwheel = CartWheel::fromWheel(wheel);
    int threshold_receive = -chargeInitial(base_cartwheel.numNeighbor()) + threshold;

    auto possible_cartwheels_within_secondneighbor = BaseWheel::decideDegreeBySendCases(base_cartwheel, send_cases, reducible_confs, max_degree, threshold_receive, true);
    ...
    for (const auto& cartwheel : possible_cartwheels_within_secondneighbor) {
        auto cartwheels = BaseWheel::decideDegreeBySendCases(cartwheel, send_cases, reducible_confs, max_degree, threshold_receive, true);
    ...
    for (const auto &cartwheel : possible_cartwheels) {
        ...
        auto [is_ovecharged, is_related] = cartwheel.isOvercharged(rules, threshold);
        if (is_ovecharged) {
...
void evaluateWheel(const string &wheel_filename, const string &rules_dirname, const string &send_cases_dirname, 
    const string &confs_dirname, int max_degree, int threshold) {
    ...
    searchOverChargedCartWheel(wheel, rules, send_cases, confs, max_degree, threshold);
    return;
}

vector<Wheel> searchPossibleOverChargedWheels(int hubdegree, const vector<Degree> &possible_degrees, 
    const vector<Configuration> &confs, const vector<Rule> &send_cases, int threshold) {
    ...
    auto decide_degree = [&](auto &&decide_degree, int v, int lowerst_deg_idx) -> void {
        ...
        if (v == hubdegree) {
            ...
            if (chargeInitial(hubdegree) + recv <= threshold) return;
...
void generateWheels(int hub_degree, const string &confs_dirname, const string &send_cases_dirname, int max_degree, int threshold, const string &output_dirname) {
    ...
    vector<Configuration> confs_filtered;
    std::copy_if(confs.begin(), confs.end(), std::back_inserter(confs_filtered), [](const Configuration &conf){
        return conf.diameter() <= 2;
    });
    spdlog::info("confs.size(): {}", confs.size());
    spdlog::info("confs_filtered.size(): {}", confs_filtered.size());

    spdlog::info("calculating wheel which does not contain conf...");
    auto wheels = searchPossibleOverChargedWheels(hub_degree, possible_degrees, confs_filtered, send_cases, threshold);
