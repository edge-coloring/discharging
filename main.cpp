...
int main(const int ac, const char* const* const av) {
    ...
    description.add_options()
        ...
        ("threshold,t", value<int>()->default_value(0), "Threshold of charge")
    ...
    if (vm.count("degree")) {
        ...
        assert(degree.fixed());
        generateWheels(degree.lower(), confdir, send_casedir, max_degree, threshold, outdir);   
    }
    if (vm.count("wheel")) {
        ...
        if (fs::path(filename).extension() == ".wheel") {
            evaluateWheel(filename, rulesdir, casesdir, confsdir, max_degree, threshold);
        }
    }
