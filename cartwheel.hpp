...
public:
    ...
    bool operator==(const CartWheel &right) const;
    void extendThirdNeighbor(void);
    pair<bool, vector<bool>> isOvercharged(const vector<Rule> &rules, int threshold) const;
};

int chargeInitial(int degree);
void evaluateWheel(const string &wheel_filename, const string &rules_dirname, const string &send_cases_dirname, const string &confs_dirname, int max_degree, int threshold);
