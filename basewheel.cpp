...
vector<ContainResult> BaseWheel::containSubgraphWithCorrespondingEdge(
    ...
    return BaseWheel::containSubgraphWithCorrespondingEdge(wheelgraph, subgraph, edge_wheelgraph, edge_subgraph, except_vertices, detect_possible);
}

// 上の containSubgraphWithCorrespondingEdge の辺番号ではなく、辺自体を指定するバージョン
vector<ContainResult> BaseWheel::containSubgraphWithCorrespondingEdge(
    const NearTriangulation &wheelgraph, const NearTriangulation &subgraph, 
    const pair<int, int> &edge_wheelgraph, const pair<int, int> &edge_subgraph, 
    const set<int> &except_vertices, bool detect_possible) {
    assert(wheelgraph.VtoV()[edge_wheelgraph.first].count(edge_wheelgraph.second));
    assert(subgraph.VtoV()[edge_subgraph.first].count(edge_subgraph.second));
    // 返り値
...
// ring の頂点を除いて conf が　wheelgraph に含まれているか判定する。
// ただし、wheelgraph の頂点 v_wheel と conf の頂点 v_conf が対応しているような含まれ方しか考えない
bool BaseWheel::containConfCorrespondingVertices(const NearTriangulation &wheelgraph, const Configuration &conf, int v_wheel, int v_conf) {
    set<int> ring_vertices;
    if (conf.ringRemains()) {
        for (int v = 0;v < conf.ringSize(); v++) ring_vertices.insert(v);
    }
    for (int edgeid_wheelgraph = 0;edgeid_wheelgraph < (int)wheelgraph.edges().size(); edgeid_wheelgraph++) {
        auto ew = wheelgraph.edges()[edgeid_wheelgraph];
        if (ew.first == v_wheel) {
            for (int edgeid_conf = 0;edgeid_conf < (int)conf.nearTriangulation().edges().size(); edgeid_conf++) {
                auto ec = conf.nearTriangulation().edges()[edgeid_conf];
                if (ec.first != v_conf) continue;
                if (BaseWheel::numOfSubgraphWithCorrespondingEdge(wheelgraph, conf.nearTriangulation(), edgeid_wheelgraph, edgeid_conf, ring_vertices) > 0) {
                    return true;
                }
            }
...
bool containOneofConfs(const NearTriangulation &wheelgraph, const vector<Configuration> &confs) {
    for (const auto &conf : confs) {
        if (BaseWheel::containConf(wheelgraph, conf)) return true;
    }
    return false;
}
...
// WheelLike には Wheel, CartWheel などの型を代入しうる
